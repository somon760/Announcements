...
const wagonByName = {
  ...
  "Calistra Vey": ["Red", "teamster", "covered wagon", 2], "Merrik Sable": ["Red", "guard", "covered wagon", 2],
  ...
  "Anwen Rusk": ["Player", "driver", "blue wagon", 4],
  "Hessan Grimhorn": ["Golden", "teamster", "covered wagon", 5], "Varkesh Ashscale": ["Golden", "guard", "covered wagon", 5],
  ...
  "Darrik Vane": ["Black", "teamster", "covered wagon", 7], "Seris Thornwake": ["Black", "guard", "covered wagon", 7],
...
const wagonLabel = wagon => wagon === "Player" ? "Blue Wagon" : `${wagon} Wagon`;
const renderPortrait = (record, wagon) => npcImages[record.name]
  ? `<div class="portrait-wrap npc-portrait"><img src="${npcImages[record.name]}" alt="${safeText(record.name)}"><span>${safeText(wagonLabel(wagon))}</span></div>`
  : wagonImages[wagon]
...
function renderRoster(filter = "all") {
  ...
  rosterGrid.innerHTML = records.map(record => {
    ...
    return `<button type="button" class="person-card" data-name="${safeText(record.name)}">${renderPortrait(record, wagon)}<div class="card-copy"><p class="card-group">${safeText(wagonLabel(wagon))}</p><h3>${safeText(record.name)}</h3><p class="role">${safeText(role)}</p><p class="tone">${safeText(description)}</p><span class="open-record">Ask about this traveler <b>↓</b></span></div></button>`;
  }).join("");
...
function renderConversation(record) {
  ...
  const notes = [["Parting words", record.quitMessage], ["If you press the subject", record.contextFallbackMessage], ["On another subject", record.fallbackMessage]].filter(([, value]) => value).map(([label, value]) => `<p class="conversation-note"><b>${label}</b>${safeText(value)}</p>`).join("");
  return `<p class="record-kicker">${safeText(wagonLabel(wagon))} · ${safeText(role)} · ON THE ROAD</p><h3 class="conversation-title">${safeText(record.name)}</h3><p class="record-intro">${safeText(description)}. Ask about the road, the work, and the lives woven into this long journey north.</p><div class="conversation-topics">${topics}</div>${notes ? `<div class="conversation-notes">${notes}</div>` : ""}`;
}
...
function renderVoteTally() {
  ...
  voteGrid.innerHTML = voteWagons.map(wagon => {
    ...
    const full = votes >= 4;
    return `<article class="vote-card"><div class="vote-wagon-image"><img src="${wagonImages[wagon]}" alt="${safeText(wagonLabel(wagon))}"></div><div class="vote-card-head"><div><p class="vote-wagon-label">${safeText(wagonLabel(wagon))}</p><h3>${votes}</h3></div><span class="vote-count-label">${votes === 1 ? "1 party mark" : `${votes} party marks`}</span></div><div class="vote-meter" aria-hidden="true"><span style="width:${votes * 25}%"></span></div><div class="vote-actions"><button type="button" class="vote-button" data-vote-wagon="${safeText(wagon)}" data-vote-action="add" ${full ? "disabled" : ""}>${full ? "Party has marked this wagon" : `Mark ${safeText(wagonLabel(wagon))}`}</button><button type="button" class="vote-button vote-button-remove" data-vote-wagon="${safeText(wagon)}" data-vote-action="remove" ${votes === 0 ? "disabled" : ""}>Remove a vote</button></div></article>`;
  }).join("");
  ...
  voteGrid.querySelectorAll("[data-vote-wagon]").forEach(button => button.addEventListener("click", () => castVote(button.dataset.voteWagon, button, button.dataset.voteAction)));
}
...
async function castVote(wagon, button, action = "add") {
  button.disabled = true;
  try {
    const response = await fetch(voteApi, { method: "POST", headers: { "Content-Type": "application/json", Accept: "application/json" }, body: JSON.stringify({ wagon, action }) });
    const data = await response.json();
  ...
  transcriptList.innerHTML = caravanConversations.map((record, index) => `<details class="transcript" id="${slugify(record.name)}" ${index === 0 ? "open" : ""}><summary><span class="transcript-count">${String(index + 1).padStart(2, "0")}</span><span><b>${safeText(record.name)}</b><small>${safeText(wagonLabel(recordWagon(record)[0]))} · On the road</small></span><i>+</i></summary><div class="transcript-body">${renderConversation(record)}</div></details>`).join("");
